...
def _latin_gate(rep: "Report", doc: dict) -> None:
    ...
    -> REVIEW (карантин LATIN_UNRESOLVED): в обучение не едет.

    Замены needs_review НЕ применены к тексту (контракт _apply: текст меняет только auto);
    исходный порченый токен ОСТАЛСЯ, спан в очереди на человека. Выводятся числом как
    LATIN_REVIEW — предупреждение (документ содержит неподтверждённую латиницу-предложение)."""
    ...
    if nr:
        rep.warn("LATIN_REVIEW",
                 "%d латинских замен помечены needs_review (НЕ применены к тексту, "
...
def validate_doc(name: str, doc: dict, toc: Optional[Toc]) -> Report:
    ...
    # ---- гейт latin recovery (13b ШАГ 8): неразрешённые КРИТИЧЕСКИЕ латинские сущности
    # -> REVIEW (карантин). Инертен без блока latin_recovery. ПОДКЛЮЧЁН после внешнего
    # ревью 2026-07-16: определение было, вызова НЕ БЫЛО -> 130 критических спанов в 88 док.
    # имели PASS. Правило I30: «гейт работает» = есть ВЫЗОВ, а не определение.
    _latin_gate(rep, doc)
