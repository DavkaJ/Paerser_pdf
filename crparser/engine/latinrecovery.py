...
# Сегментация whitespace-токена: ДЕФИС, пробелы, ОДИНОЧНЫЙ слэш, `+`, скобки `()`.
# `.` и `[` не режем (коды `037.6`/`S01EC` и порча `1п[епог`/`8Иа//ег` целы). `+`/`()` —
# границы склеек «препарат+[комбинация», «слово)[цитата», «Пиперациллин+[Тазобактам» ->
# режем, чтобы не латинизировать склеенные рус. слова (Group B, КР66_4). ОДИНОЧНЫЙ `/` —
...
def _cyr_orphan(core: str) -> bool:
    ...
    # 2) скобка/двойной-слэш/$ ВНУТРИ токена со строчными И цифрой (8Иа//ег, 1п[епог) —
    #    глиф-ошибка. Требуем цифру: `стерильность)[80`/`слово[цит` (склейка рус.+цитата,
    #    `)`/`+` уже вырезаны сегментацией) без цифры-В-СЛОВЕ не латинизируем (Group B).
    if re.search(r"[\[\]/$]", core[1:]) and _DIGIT.search(core):
        return True
    # 3) camelCase со ЗНАЧИМЫМ телом слова: >=3 подряд строчных, затем ЗАГЛАВНАЯ (ТетрогаШ:
    #    «етрога»+Ш). Русские mixed-case аббревиатуры (АтД/СПбГПМУ) имеют <3 подряд строчных.
    if re.search(r"[а-яёa-z]{3,}[А-ЯЁA-Z]", core):
        return True
    # 4) цифра ВНУТРИ (не в конце) слова, букв БОЛЬШЕ чем цифр (8рае1И: 1 внутри, букв 4).
    #    Отсекает: число+буква (103н), хвостовые сноски (Дискератоз7), ЧИСЛО+РУС.СЛОВО
    #    (`19лет`/`80мл` — склейка, чистое рус. слово; фичим только его, не латинизируем).
    if (len(letters) >= 3 and len(letters) > n_digit and _DIGIT.search(core[1:-1])
