...
# ============ ЗАКРЫТЫЙ НАУЧНЫЙ ЛЕКСИКОН (газы/КЩС/белки) ============
# Кириллическая запись ОДНОЗНАЧНОГО научного токена: `СО2`(=CO2), `рН`(=pH),
# `р53`(=p53). Класс порчи тот же, что A2 (I19) — шрифт и /ToUnicode честны, автор
# набрал латинскую формулу с русской раскладки, — но форма НЕ медкод, поэтому шаблон
# `normalize_code_token` её не берёт. Хуже: ICD-регексп `_looks_code` матчит `CO2`
# (`[A-Za-z][0-9OoОо][0-9OoОо]`), и `СО2` уезжала в `unresolved_critical` — 16 из 41
# флагнутых документов держались одним углекислым газом. Ни один канал починить её не
# мог: шрифт честен (font-repair молчит), а eng-OCR читает ровно тот глиф, что нарисован.
#
# СПИСОК ЗАКРЫТЫЙ И РЕГИСТРОЗАВИСИМЫЙ. Регистр — не косметика, а ГЕЙТ ТОЧНОСТИ
# (замер по корпусу outout_latin_v2, 722 док., аудит всех вхождений против источника):
#   * `рН` (строчная р + заглавная Н) = pH — 370 вхождений, ВСЕ кислотно-щелочные;
#   * `РН` (ALL-CAPS) = РУССКАЯ АББРЕВИАТУРА — 254 вхождения: «ретинопатия
#     недоношенных» (КР107_2, ~170 шт.), «радикальная нефрэктомия» (КР10_5, ~40),
#     «реакция нейтрализации» (КР911_1, КР567_2), плюс латинские инициалы в
#     библиографии (`de Jongh РНС`, `PHQ-2`->`РНО-2`). Регистро-НЕзависимый список
#     латинизировал бы все 254 — ровно класс тихой порчи, который ловит правило 4;
#   * `со2` (строчная) = русское «со 2-3 дня после операции» (КР691_2), не CO2.
# ИСКЛЮЧЕНЫ ПОСЛЕ АУДИТА (не по форме, а по вхождениям):
#   * `O2` — 3 ложных из 35: пункт анкеты «О2. ТРЕВОГА» (КР451_3), «ИФН-о2» (КР627_3),
#     «НЭО легких и тимуса О2- 3» (КР610_2). Двух символов на однозначность не хватает;
#   * `p14` — 2 вхождения, НИ ОДНО не белок: `рТ, р14, рМ` (это pN, КР1_4) и сноска
#     в таблице наследственных синдромов (КР229_3).
# ДОБАВЛЕН СВЕРХ ИСХОДНОГО СПИСКА: `p27` — `р27(Kip1)` (КР88_5), ингибитор CDK,
# однозначный белок.
_SCI_LEXICON = frozenset({
    # газы и кислотно-щелочное состояние
    "CO2", "H2O", "pH",
    "pO2", "PO2", "pCO2", "PCO2",
    "PaO2", "paO2", "PaCO2", "paCO2",
    "SpO2", "SaO2", "FiO2",
    # белки-маркеры (все — реальные вхождения корпуса, см. аудит)
    "p16", "p24", "p27", "p53",
})


def sci_lexicon_form(core: str, tnm_ctx: bool = False) -> Optional[str]:
    """Кир-гомографная запись токена закрытого научного лексикона -> латиница.
    Детерминированно, без OCR: замена только БУКВ-гомографов, длина и регистр целы.

    ГЕЙТ TNM-РЕГИОНА обязателен. В регионе стадирования `p`+буква — это
    ПАТОЛОГОАНАТОМИЧЕСКАЯ КАТЕГОРИЯ, а не формула: `категории рТ, рН рМ соответствуют
    категориям Т, 14, М` (КР450_3) — здесь `рН` это pN, а не pH. Замер: из 625
    кандидатов `pH` по корпусу ровно ОДИН лежит в TNM-регионе, и он — ровно этот
    ложный. Гейт стоит корпусу 0 верных замен и снимает 100% известных ложных.
    """
    if tnm_ctx or len(core) < 2 or not _CYR_ANY.search(core):
        return None
    lat = "".join(_CYR2LAT.get(c, c) for c in core)
    if lat == core or _CYR_ANY.search(lat):
        return None                            # осталась кириллица -> русское слово
    return lat if lat in _SCI_LEXICON else None
...
class LatinRecoverer:
    ...
    def recover(self, sections: List, tables: List, excluded: Dict,
        ...
        for core in core_seen:
            ...
            # ЗАКРЫТЫЙ НАУЧНЫЙ ЛЕКСИКОН — ДО классификации кода. Иначе `СО2` матчится
            # ICD-шаблоном и уходит в unresolved_critical: детектор прав (порча реальна),
            # но резолюции у него нет ни в одном канале. Здесь ответ детерминирован.
            sci = sci_lexicon_form(core, tnm_ctx)
            if sci:
                self._g[core] = _mkrec(core, sci, "sci_lexicon",
                                       "closed-scientific-lexicon", 1.0, "term",
                                       False, "auto", ["sci_homoglyph"],
                                       candidates=[sci])
                continue
