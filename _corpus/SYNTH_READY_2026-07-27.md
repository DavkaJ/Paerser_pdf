...
- Остаток `unresolved_critical` (60 форм в 29 док.) — это РАССЫПАННЫЕ ATC-коды
  (`СО1СА24`=C01CA24, `МО1АЕ01`=M01AE01): кир-гомограф буквы И цифро-глиф `О`→`0`
  в одном токене. Отдельный класс, лексиконом не закрывается.

---

## 7. Задача 1 (2026-07-28): закрытый научный лексикон

Прогон `20260727T194612Z_16719e70`, 722 док., 1249 с, `run_integrity_ok=True`.

`СО2`/`рН`/`р53` с кириллическими гомографами разрешаются детерминированно ДО
классификации кода (`latinrecovery.sci_lexicon_form`) — раньше ICD-регексп матчил
`CO2` и форма уезжала в `unresolved_critical`, а починить её не мог ни один канал.

| Метрика | Было | Стало |
|---|---|---|
| Валидатор | PASS 502 / REVIEW 158 / FAIL 62 | **PASS 521 / REVIEW 139 / FAIL 62** |
| `unresolved_critical` (форм / док.) | 84 / 50 | **60 / 29** |
| Тиры пригодности | A 156 / B 515 / C 51 | **A 160 / B 511 / C 51** |
| Флаг `unresolved_critical` в выгрузке | 41 док. | **20 док.** |
| `tables_found` / `sections_found` / coverage | 11 597 / 21 603 / — | без изменений |

**Документов, потерявших статус: 0** (19 REVIEW→PASS, остальные без движения).
557 авто-замен, полный аудит против источника — `_corpus/_sci_lexicon_audit.md`
(регистрозависимость обязательна: ALL-CAPS `РН` — русская аббревиатура в 254 местах;
`O2` и `p14` исключены по вхождениям; `p27` добавлен).

Побочная находка A/B: старый пайплайн в 9 местах коэрцил `СО2` в **ICD-код `C02`**
(злокачественное новообразование языка) — теперь там `CO2`. Ещё `P24`/`P16`/`P53`
(неверный регистр от eng-OCR) стали `p24`/`p16`/`p53`.

---

## 8. Задача 2 (2026-07-28): остаток PUA разобран по шрифту

Прогон `20260727T202425Z_34ae620a`, 722 док., 1011 с, `run_integrity_ok=True`.

Остаток вне таблицы Adobe Symbol разрешён по СЕМЕЙСТВУ ШРИФТА, которым глиф
нарисован (кодпоинтный ремап был бы порчей: `U+F0EA` — это `⬇` в Wingdings и `★` в
Wingdings 2). Каждое правило получено рендером глифа из реального PDF —
`_corpus/_pua_font_remap.md`.

| Метрика | Было | Стало |
|---|---|---|
| PUA-глифов в корпусе | 1 579 в 47 док. | **19 в 2 док.** |
| Кодпоинтов без обоснования | 8 | **2** |
| Тиры пригодности | A 160 / B 511 / C 51 | **A 166 / B 505 / C 51** |
| Флаг `needs_review_spans` | 205 док. | **174 док.** |
| Валидатор, `tables_found`, `sections_found`, coverage | — | без изменений |

Замена 1:1 (длина текста цела): ушло 1 573 PUA, пришло `☐`×808, `✓`×399, `●`×229,
`—`×97, `⬇`×12, `■`×7, `⬆`×4, `★`×1. Изменилось 55 документов (47 с PUA + 8 —
известный недетерминизм OCR-каналов, ±1 токен в обе стороны). Статус не изменил
ни один документ.

Осталось `needs_review` только необоснованное: `U+F0FC`×17 в Times New Roman
(КР1039_1, глиф не рисуется) и `U+F0EA`×2 в КР660_2 (документ рисует кодпоинт
двумя семействами сразу — гейт неоднозначности).

---

## 9. Задача 3 (2026-07-28): «огрызки таблиц» — класс не подтвердился

Перепарс НЕ потребовался (менялся критерий, не выход движка): перевалидация 41 с +
перевыгрузка. Полный замер — `_corpus/_table_stub_measure.md`,
скрипт `_corpus/measure_table_stubs.py`.

Гипотеза «вырезана шапка, тело потеряно» опровергнута тремя независимыми проверками:

1. `raw_text` содержит ВЕСЬ текст своего bbox у **1 010 из 1 016** коротких таблиц
   (оставшиеся 6 — документы с битым текстовым слоем, они уже в карантине);
2. таблиц с `row_count<=1` (шапка без тела) в корпусе **0** — движок их не выпускает;
3. из коротких таблиц, под которыми продолжается рамка, **ни у одной** нет табличного
   текста ниже bbox: 830 — рамка кончается на bbox, 96 — ниже проза/сноска,
   90 — ниже ДРУГОЙ табличный объект (расщепление, текст не потерян).

Критерий «короче 200 символов» ловил не то: **991 из 1 016** помеченных таблиц не
несут ни одной улики порчи (плотная числовая таблица на 8 строк укладывается в 200
символов), и при этом критерий **пропускал 841** таблицу, которую сам движок пометил
`low_confidence`. Флаг переведён на улику движка.

| Метрика | Было | Стало |
|---|---|---|
| Таблиц `low_confidence` в выгрузке | 1 016 в 379 док. | **739 в 219 док.** |
| Тиры пригодности | A 166 / B 505 / C 51 | **A 309 / B 362 / C 51** |
| Флаг `table_stubs` | 355 док. | **219 док.** |
| Статусы валидатора, `tables_found`, `sections_found`, coverage | — | без изменений |

Осталось замеренным, но не почищенным: **90 расщеплений** (одна визуальная таблица
извлечена двумя объектами). Склейка уменьшит `tables_found` — решение за владельцем.
