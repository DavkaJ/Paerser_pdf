...
# Гомоглифы ВЫВОДИМ ИЗ КАНОНИЧНОЙ КАРТЫ ДВИЖКА (latinnorm._CYR2LAT), а не хардкодим —
# иначе гейт расходится с тем, как движок ОПРЕДЕЛЯЕТ гомоглиф везде (адверс-замер нашёл:
# рукодельный набор включал строчные в/к/м/н/т, которые в НИЖНЕМ регистре НЕ двойники
# латинских — их VLM читает по различимым пикселям, — и пропускал у↔y/і↔i).
# Заглавные (визуально = лат. заглавной): {Ѕ І Ј А В Е К М Н О Р С Т У Х}.
# Строчные  (визуально = лат. строчной):  {а е о р с у х і}.
_HOMO_UP = frozenset(k for k in _CYR2LAT if k.isupper())
_HOMO_LOWER = frozenset(k for k in _CYR2LAT if k.islower())
...
def _source_is_russian(source: str) -> bool:
    ...
    # ОТРЕЗАТЬ ПРИКЛЕЕННУЮ ЦИТАТУ/ЗНАЧЕНИЕ (склейка токенизатора, I20): `МАНК[13`->`МАНК`,
    # `МНО>1,5`->`МНО`. Цифра ПОСЛЕ скобки/оператора — внешняя ссылка, НЕ корруптор буквы;
    # без этого whole-core-тест счёл бы рус. аббревиатуру с цитатой битой латиницей и
    # латинизировал бы её (МАНК->MAHK, МНО->MHO — клинически значимо: МНО=МНО свёртываемости).
    seg = re.split(r"[\[\]<>=(){},;]", core)[0].strip("'\"«».,;:-/ ")
    if seg:
        core = seg
    low = core.lower()
    ...
    if _RX_LONG_LOWER_CYR.match(core):
        return True
    # ПРИНЦИП правил 3/3b (пиксель-двусмысленность): токен блокируется из auto, ТОЛЬКО если
    # ВЕСЬ он (каждый символ core) — гомоглиф. ЛЮБОЙ различимый символ — не-гомоглифная буква
    # (`ЕТОКЯ` несёт Я, `МЕОЫЫЕ` — Ы) ИЛИ ЦИФРО-ГЛИФ порчи (`ККА8`=KRAS: 8<-S; `8атр1е`=Sample:
    # 8/1; `ос1е1у`=Society: 1) — делает пиксели ОДНОЗНАЧНЫМИ: рус. аббревиатура НЕ содержит
    # 8/1/0//, значит это заведомо битая ЛАТИНИЦА -> на AND-гейт, НЕ блокируем.
    # (Адверс-замер: тест ТОЛЬКО по alpha-буквам переблокировал 42 задачи битой латиницы с
    # цифро-глифами; тест по ВСЕМУ core их освобождает, 10 штатных гомоглиф-целей сохраняет.)
    if not _CYR_ANY.search(core):
        return False
    letters = [c for c in core if c.isalpha()]
    # 3. ГОМОГЛИФ-ONLY ALL-CAPS (нет строчных букв) — рус. аббревиатура из букв-двойников
    #    (`МАНК`=метод амплиф.нукл.кислот, `МНО`=межд.нормализ.отношение). Обе визуальные ноги
    #    AND-гейта КОРРЕЛИРОВАНЫ (транслитерируют одинаково, I28) -> различить язык пикселями
    #    нельзя, держит только это правило. `ЕТОКЯ`(ETDRS)/`МЕОЫЫЕ`(MEDLINE)/`ККА8`(KRAS) НЕ
    #    гомоглиф-only (несут Я/Ы/цифру) -> в auto. `ГЦР`/`СОЭ`/`СРБ` — Г/Ц/Э/Б без двойника.
    if (not any(c.islower() for c in core) and len(letters) >= 2
            and all(c in _HOMO_UP for c in core)):
        return True
    # 3b. СТРОЧНЫЙ гомоглиф-only, 3-6 букв — тот же профиль, строчный. Строчные двойники —
    #     {а е о р с у х і} (из _CYR2LAT): заглавные К/М/Н/Т в НИЖНЕМ регистре РАЗЛИЧИМЫ, VLM
    #     читает их верно -> НЕ блокируем (`рат`/`скн` — не гомоглиф-only строчно, на AND-гейт).
    #     Форма (7+) + это (3-6) = строчные ЧИСТО-гомоглифные >=3. Целевой скан: закрывает
    #     `асое`->eptacog, `оту`/`уро`/`еуе` (с у↔y из карты); дыра у↔y была из-за хардкода.
    if (core.islower() and 3 <= len(letters) <= 6
            and all(c in _HOMO_LOWER for c in core)):
        return True
    return False
...
def and_gate(verdict, cands, indep, kind, source=""):
    ...
    if _source_is_russian(source):
        return "queue", cand
    # КЛИНИЧЕСКИЙ СТОП для СТАДИЙ: нога 2 (_ocr_confirms) НЕ локализована — Tesseract идёт
    # psm=7 по ВСЕЙ строке, и entity_valid('stage') принимает ЛЮБОЙ роман. На построчном
    # кропе кандидат-стадия может «подтвердиться» СОСЕДНЕЙ стадией строки: замер пилота нашёл
    # `1-Н`(=I-II, ранняя) -> `III-IV`(поздняя, соседа) — подмена, меняющая курабельность.
    # Пока кроп построчный — стадию в auto НЕ пускаем (только очередь+подсказка). ПРОД чинит
    # это ПОСЛОВНЫМ кропом (OCR читает лишь целевое слово -> сосед не подтверждает); тогда
    # снять этот стоп и вернуть строгую сверку роман-структуры источника с кандидатом (I27).
    if kind == "stage":
        return "queue", cand
    ...
    k = kind if kind in ("icd", "atc", "tnm") else "term"
